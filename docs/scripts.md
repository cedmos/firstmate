...
| `fm-crew-worktree-instructions-lib.sh` | Overlay crewmate project instructions in a firstmate-repo ship or scout worktree, and refuse session start there |
| `fm-crew-instructions.sh` | Worker-runnable status, removal, and per-task edit recovery for that crew overlay   |
