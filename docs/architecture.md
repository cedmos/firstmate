...
Removal is the same library's job and runs before every pooled-worktree refresh in `bin/fm-spawn.sh`, so a slot returned with the overlay still installed self-heals instead of wedging the next `git reset --hard` on an invisible skip-worktree entry; it also folds any saved sidecar back onto its instruction file rather than deleting it, because the sidecar is the only durable copy of the edits the overlay displaced and `git status` cannot see it; the work becomes ordinary uncommitted work, so the refresh's clean check refuses the slot instead of discarding it, and the next worker in that slot never inherits the sidecar.
