...
That setting names the whole hook set rather than one entry in it, so the install remembers the path it displaced, chains the guard to that path's `pre-commit` once the overlay is unstaged, forwards its other hooks unchanged, and puts a displaced worktree-scoped value back on removal instead of unsetting it.
An instruction file the worker deleted while `HEAD` still tracks it refuses the install rather than being passed over, because writing the overlay over that path would recreate the file and destroy the deletion, while skipping it leaves the launch with no crew instructions at all.
