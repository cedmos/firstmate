...
config/pi-supervision-branch  Pi-only toggle for the in-process supervision branch that absorbs fleet wakes on a Pi primary; LOCAL, gitignored, not inherited; absent or "on" = enabled, anything else = today's wake-to-main behavior; see docs/configuration.md "Pi supervision branch" and docs/pi-supervision-branch.md
...
state/               runtime records and signals; gitignored
  ...
  branch-pending-wakes/ .pi-branch-generation  accepted Pi branch dispatches awaiting completion and the active lease-generation fence; extension-owned (docs/pi-supervision-branch.md)
