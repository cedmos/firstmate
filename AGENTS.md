...
config/pi-supervision-branch  Pi-only toggle for the in-process supervision branch that absorbs ordinary actionable fleet wakes on a Pi primary; LOCAL, gitignored, not inherited; absent or "on" = enabled, anything else = today's wake-to-main behavior; see docs/configuration.md "Pi supervision branch" and docs/pi-supervision-branch.md
